...
public class ListBox< T > extends Panel
{
	...
	public ListBox( )
	{
		this( true, new VersionedList< T >( ), null, new SelectionModel( ), new ElementId( ELEMENT_ID ), null );
	}
	...
	protected ListBox( boolean applyStyles, VersionedList< T > model, CellRenderer< T > cellRenderer, SelectionModel selection,
			ElementId elementId, String style )
	{
		...
		grid.getControl( GuiControl.class ).addComponent( sizerIndex, "sizer", new AbstractGuiComponent( )
		{
			@Override
	...
	public static void initializeDefaultStyles( Styles styles, Attributes attrs )
	{
		...
		// QuadBackgroundComponent quad = new QuadBackgroundComponent(new
		// ColorRGBA(0.5f, 0.5f, 0.5f, 1));
		QuadBackgroundComponent quad = new QuadBackgroundComponent( new ColorRGBA( 0.8f, 0.9f, 0.1f, 1 ) );
		// quad.getMaterial( ).getMaterial( ).getAdditionalRenderState(
	...
	public void setModel( VersionedList< T > model )
	{
		...
		grid.setModel( new GridModelDelegate( ) ); // need a new one for a new
													// version
		resetModelRange( );
